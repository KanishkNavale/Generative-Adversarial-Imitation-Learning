...
class Agent:
    def __init__(self, env, datapath, n_games, alpha=0.001,
                 beta=0.002, gamma=0.99, tau=0.01, batch_size=32,
    ...
    def update_noisy_actor(self):
        with T.no_grad():
            for actor_weights, noisy_actor_weights in zip(self.actor.parameters(), self.noisy_actor.parameters()):
                shape = actor_weights
                noise = T.normal(mean=T.zeros_like(
                    shape), std=self.scalar * T.ones_like(shape)).to(device)
    ...
    def choose_action(self, observation):
        ...
        if self.noise == 'normal':
            action += T.normal(mean=T.zeros_like(action, dtype=T.float32),
                               std=self.noise_param * T.ones_like(action, dtype=T.float32)).to(device)
    ...
    def save_models(self):
        ...
        if self.noise == 'param':
            self.noisy_actor.save_model(self.datapath)
    ...
    def load_models(self):
        ...
        if self.noise == 'param':
            self.noisy_actor.load_model(self.datapath)
